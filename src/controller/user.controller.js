...
const userController = {
  ...
  registerUser: async (req, res) => {
    ...
    if (password !== confirmPassword) {
      return res.status(400).json({ message: "Confirm password do not match with password" });
    }
    ...
    try {
      ...
      if (user.rows.length > 0) {
        ...
        if (!match) {
          ...
          const accessToken = jwt.sign({ userId, username, email, photo_path, level }, process.env.ACCESS_TOKEN_SECRET, {
            expiresIn: "30s",
          });
          const refreshToken = jwt.sign({ userId, username, email, photo_path, level }, process.env.REFRESH_TOKEN_SECRET, {
            expiresIn: "1d",
          });
          ...
          res.json({ userId, username, email, photo_path, accessToken, message: "Login successfully" });
        }
      } else {
