...
const userModel = {
  ...
  refreshTokenUpdate: (token, id) => {
    return new Promise((resolve, reject) => {
      db.query(`UPDATE users SET refresh_token = '${token}' WHERE id = ${id}`, (err, res) => {
        if (err) {
          reject(err)
        }
        resolve(res)
  ...
  findUserRefreshToken: (token) => {
    return new Promise((resolve, reject) => {
      db.query(`SELECT * FROM users WHERE refresh_token = '${token}'`, (err, res) => {
        if (err) {
          reject(err)
        }
        resolve(res)
  ...
  updateUser: ({ id, username, email, password }) => {
    return new Promise((resolve, reject) => {
      db.query(`UPDATE users SET username='${username}', email='${email}', password='${password}' WHERE id=${id}`, (err, result) => {
        if (err) {
          reject(err)
        }
        resolve(result)
  ...
  updateProfile: ({ id, username, photoPath }) => {
    return new Promise((resolve, reject) => {
      db.query(`UPDATE users SET username='${username}', photo_path='${photoPath}' WHERE id=${id}`, (err, result) => {
        if (err) {
          reject(err)
        }
        resolve(result)
  ...
  updateUsername: ({ id, username }) => {
    return new Promise((resolve, reject) => {
      db.query(`UPDATE users SET username='${username}' WHERE id=${id}`, (err, result) => {
        if (err) {
          reject(err)
        }
        resolve(result)
